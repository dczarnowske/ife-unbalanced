######################################### Online Supplement ########################################
# Results from Monte Carlo Simulations (Section 4) with Algorithm 2
#
####################################################################################################
...
# helper variables
criteria <- paste0(c("ic2", "bic3", "er", "gr", "ed", "pa"), "_rescale_rd")
...
table1 <- dt[, .(
  L       = first(L),
  bias    = mean((beta_rd - rho) / rho) * 100.0,
  ratio   = mean(se_beta_rd) / sd(beta_rd),
  size    = mean(abs(beta_rd - rho) / se_beta_rd > qnorm(0.975))
), by = configs]
